...
public final class ODEntityListener implements Listener {
	...
	private void blowBlockUp(final Location at) {
		...
		if (b.getTypeId() == 49 || b.getTypeId() == 116 || b.getTypeId() == 130) {
			...
			if (config.getDurabilityEnabled() && config.getoDurability() > 1) {
	...
	private void destroyBlockAndDropItem(final Location at) {
		...
		if (chance == 1.0 || chance <= random) {
			...
			/*
			 * New way to drop due to item stack deprecration.
			 * Only drops if enchantment table or obsidian.
			 * Item stack is always singular
			 */
		...
		// Was not always passing to here.
	}

	private boolean checkIfMax(int value) {
		return value == config.getoDurability();
	}
